<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:82:"E:\phpStudy\WWW\sugoo\admin\public/../application/index\view\goods\addproduct.html";i:1515050159;}*/ ?>
...
<div id="urHere">DouPHP 管理中心<b>></b><strong>添加商品</strong> </div>   <div class="mainBox" style="height:auto!important;height:550px;min-height:550px;">
            <h3><a href="<?php echo url('goods/index'); ?>" class="actionBtn">商品列表</a>添加商品</h3>
    <form action="<?php echo url('goods/goodsAdd'); ?>" method="post" enctype="multipart/form-data">
  ...
  $(document).on('change','#c_id',function(){
    ...
    $.ajax({
      ...
      success:function(msg)
      {
        ...
        for(i in msg)
        {
           html+="<span>"+msg[i].attr_name+"："+"</span>"
           var data = msg[i].attra_val;
            for(k in data)
            {
               html+="<input type=\"checkbox\" name=\"attrvalue["+n+"][]\" value="+data[k].attra_val+">"
               html+=""+data[k].attra_val+""
            }
            html+="<br/>"
            n++;
            $('#attr').html(html);
